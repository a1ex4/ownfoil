...
def _use_progress_bar(statusReportInfo):
	return statusReportInfo == None and not lowVerbose and sys.stdout.isatty()
...
def __decompressNcz(nspf, f, statusReportInfo, pleaseNoPrint):
	...
	if _use_progress_bar(statusReportInfo):
	...
	elif _use_progress_bar(statusReportInfo):
	...
	for s in sections:
		...
		while i < end:
			...
			elif _use_progress_bar(statusReportInfo) and decompressedBytes - decompressedBytesOld > 52428800: #Refresh every 50 MB
				decompressedBytesOld = decompressedBytes
				bar.count = decompressedBytes//1048576
				bar.refresh()
