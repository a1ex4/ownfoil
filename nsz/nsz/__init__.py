...
def solidCompressTask(in_queue, statusReport, readyForWork, pleaseNoPrint, pleaseKillYourself, id, problemQueue, keysFile):
	...
	while True:
		...
		try:
			...
			if verifyArg:
				Print.status("[VERIFY]    {0}".format(outFile), pleaseNoPrint)
				try:
...
def compress(filePath, outputDir, args, work, amountOfTastkQueued):
	...
	if filePath.suffix == ".xci" and not args.solid or args.block:
		...
		if args.verify:
			Print.status("[VERIFY]    {0}".format(outFile))
			try:
...
def main():
	...
	try:
		...
		keysFile = getattr(args, "keys_file", "")
		keysFile = keysFile if keysFile else None
		if keysFile:
			if not Path(keysFile).is_file():
				Print.error(107, 'Error: Keys file "{0}" does not exist!'.format(keysFile))
				return
			keys_loaded = Keys.load(keysFile)
		else:
		...
		if args.C:
					...
					try:
						...
						if filePath.suffix == '.nsp':
							...
							if not AllowedToWriteOutfile(filePath, ".xcz", targetDictXcz[outFolder], args):
								continue
						targetFile = targetExtension(outFolder.joinpath(filePath.name))
						Print.status('[CONVERT]   {0} -> {1}'.format(filePath, targetFile))
			...
			for i in range(parallelTasks):
				...
				p = Process(target=solidCompressTask, args=(work, statusReport, readyForWork, pleaseNoPrint, pleaseKillYourself, i, problems, keysFile))
				p.start()
				pool.append(p)

			if machineReadableOutput == False and isatty:
			...
			while readyForWork.value() < parallelTasks:
				...
				# Show the progress bar only if the output is human readable and interactive.
				if machineReadableOutput == False and isatty:
					for i in range(parallelTasks):
						...
						bars[i].refresh()
				elif machineReadableOutput == False and not isatty:
					now = time.time()
					if now - lastProgressPrint >= 1:
						for i in range(parallelTasks):
							compressedRead, compressedWritten, total, currentStep = statusReport[i]
							if total <= 0:
								continue
							percent = int(compressedRead * 100 / total)
							current = (percent, currentStep)
							if lastProgressSnapshot.get(i) != current:
								if args.low_verbose:
									sys.stdout.write("{0}\n".format(percent))
									sys.stdout.flush()
								else:
									Print.info("[{0}/{1}] {2}: {3}% ({4}/{5} MiB)".format(
										i + 1,
										parallelTasks,
										currentStep,
										percent,
										compressedRead//1048576,
										total//1048576
									))
								lastProgressSnapshot[i] = current
		...
		if args.verify and not args.C and not args.D:
			for f_str in args.file:
				for filePath in expandFiles(Path(f_str)):
					try:
						if isGame(filePath):
							Print.status("[VERIFY]    {0}".format(filePath))
